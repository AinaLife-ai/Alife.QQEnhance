...
本插件独有（幼央没有）：禁言、音乐卡片、合并转发、QGetMessages 实时ID、感知通知。
...
- 引用回复使用 OneBot 结构化 `reply` 参数（`send_msg` 的 `reply` 字段），比 `[CQ:reply]` 更可靠
